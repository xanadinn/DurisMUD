...
//
// checkLastObjStrn : checks for existance of aff1 and aff2 flags for an obj,
...
//  if (readFromSubdirs) strcpy(objectFilename, "obj/");
  ...
  if (filename) strcpy(objectFilename, filename);
  else cout << "BIG OLE ERROR\n"; //strcat(objectFilename, getMainZoneNameStrn());
...
"Press any key to continue...";
 ...
 // convert em over.  fun begins.
  ...
  if (oldObjConv)
  {
    cout << "this utility does not really support mat conversion.  bye.\n";
