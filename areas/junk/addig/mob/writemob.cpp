...
void writeMobtoFile(FILE *mobFile, dikuMob *mob)
{
 ...
 // next, write the action, aff1, aff2, aff3, aff4 flags, alignment and "S"
...
//  if (readFromSubdirs) strcpy(mobFilename, "mob/");
  ...
  if (filename) strcpy(mobFilename, filename);
  else cout << "ERROR #69 LOSER\n";//strcat(mobFilename, getMainZoneNameStrn());
